...
impl Mapping {
    pub fn read<R: BitRead>(reader: &mut R, channel_count: usize, floor_count: usize, residue_count: usize) -> Result<Self> {
        ...
        let submap_count = if r#try!(reader.read_bool()) {
            r#try!(reader.read_u8_bits(4)) as usize
        } else {
            1
        };
        let has_channel_couplings = r#try!(reader.read_bool());
        let channel_couplings = if has_channel_couplings {
            let len = r#try!(reader.read_u8()) as usize + 1;
            let mut channel_couplings = Vec::with_capacity(len);
            let channel_index_bits = crate::util::Bits::ilog(channel_count as u32 - 1) as usize;
            for _ in 0..channel_couplings.capacity() {
                let mag_channel = r#try!(reader.read_u8_bits(channel_index_bits)) as usize;
                let ang_channel = r#try!(reader.read_u8_bits(channel_index_bits)) as usize;
        ...
        // Reserved.
        if r#try!(reader.read_u8_bits(2)) != 0 {
        ...
        let channel_to_submap = if submap_count > 1 {
            ...
            for _ in 0..channel_count {
                let submap_idx = r#try!(reader.read_u8_bits(4)) as usize;
        ...
        for submap_idx in 0..submap_count {
            ...
            let floor = r#try!(reader.read_u8()) as usize;
            if floor >= floor_count {
                return Err(Error::Undecodable("Invalid mapping floor value"));
            }

            let residue = r#try!(reader.read_u8()) as usize;
