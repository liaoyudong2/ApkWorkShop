...
use crate::util::{Bits, Push, Pusher2d, Pusher2dStep};
...
impl ResidueKind {
    fn from_u16(value: u16) -> Option<Self> {
        match value {
            0 => Some(Self::Residue0),
            1 => Some(Self::Residue1),
            2 => Some(Self::Residue2),
            _ => None,
        }
    }
}
...
impl Residue {
    pub fn read<R: BitRead>(reader: &mut R, codebook_count: usize) -> Result<Self> {
        let kind = if let Some(kind) = ResidueKind::from_u16(r#try!(reader.read_u16())) {
            kind
        } else {
            return Err(Error::Undecodable("Unsupported residue type"));
        };
        let start = r#try!(reader.read_u32_bits(24)) as usize;
        let end = r#try!(reader.read_u32_bits(24)) as usize;
        if end < start {
            return Err(Error::Undecodable("Invalid residue range"));
        }

        let part_len = r#try!(reader.read_u32_bits(24)) as usize + 1;
        let class_count = r#try!(reader.read_u8_bits(6)) as usize + 1;
        let classbook = r#try!(reader.read_u8_bits(8)) as usize;
        ...
        for _ in 0..class_count {
            let low_bits = r#try!(reader.read_u8_bits(3));
            let has_high_bits = r#try!(reader.read_bool());
            let high_bits = if has_high_bits {
                r#try!(reader.read_u8_bits(5))
            } else {
                0
            };
        ...
        for c in &cascade {
            ...
            for bit in 0..8 {
                if c.is_bit_set(bit) {
                    let codebook_idx = r#try!(reader.read_u8()) as usize;
            ...
            'outer: while part_count < parts_to_read {
                if pass == 0 {
                    for (i, &c) in channels.iter().enumerate() {
                        ...
                        let mut temp = r#try!(codebook.decode_scalar(reader)) as usize;
                ...
                for _ in 0..classwords_per_codeword {
                    for (i, &c) in channels.iter().enumerate() {
                        ...
                        if let Some(vq_book) = vq_book {
                            ...
                            r#try!(self.codebook_decode(&mut pusher, reader, codebook));
                        } else {
    ...
    fn codebook_decode<P: Push<f32>, R: BitRead>(&self, result: &mut P, reader: &mut R, codebook: &Codebook) -> Result<()> {
        ...
        for _ in 0..self.part_len / codebook.dim_count {
            r#try!(codebook.decode_vq(reader, result));
        }
        Ok(())
    }
}