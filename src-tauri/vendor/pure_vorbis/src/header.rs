...
impl Header {
    pub fn read<R: BitRead>(reader: &mut R) -> Result<Header> {
        if r#try!(reader.read_u32()) != 0 {
            return Err(Error::Undecodable("Unsupported Vorbis version"));
        }

        let channel_count = r#try!(reader.read_u8()) as usize;
        if channel_count == 0 {
            return Err(Error::Undecodable("Invalid channel count"));
        }

        let sample_rate = r#try!(reader.read_u32());
        if sample_rate == 0 {
            return Err(Error::Undecodable("Invalid sample rate"));
        }

        let bitrate_max = r#try!(reader.read_i32());
        let bitrate_nom = r#try!(reader.read_i32());
        let bitrate_min = r#try!(reader.read_i32());

        let frame_len_short = 1 << r#try!(reader.read_u8_bits(4)) as usize;
        if frame_len_short < 64 || frame_len_short > 8192 {
            return Err(Error::Undecodable("Invalid short frame length"));
        }
        let frame_len_long = 1 << r#try!(reader.read_u8_bits(4)) as usize;
...
impl Comments {
    pub fn read<R: BitRead>(reader: &mut R) -> Result<Self> {
        let vendor = r#try!(Self::read_string(reader));

        let comment_count = r#try!(reader.read_u32()) as usize;
        let mut comments = Vec::with_capacity(comment_count);
        for _ in 0..comment_count {
            let s = r#try!(Self::read_string(reader));
            if let Some(s) = s {
                comments.push(s);
            }
        }

        let framing_bit = r#try!(reader.read_bool());
        if !framing_bit {
            return Err(Error::Undecodable("Invalid framing bit"));
        }
    ...
    pub fn iter<'a>(&'a self) -> Box<dyn Iterator<Item=(CommentTag<'a>, &'a str)> + 'a> {
    ...
    pub fn by_tag<'a>(&'a self, tag: CommentTag<'a>) -> Box<dyn Iterator<Item=&'a str> + 'a> {
        let iter = self.iter()
    ...
    fn read_string<R: BitRead>(reader: &mut R) -> Result<Option<String>> {
        let len = r#try!(reader.read_u32()) as usize;
        let mut bytes = vec![0; len];
        r#try!(reader.read_exact(&mut bytes));
