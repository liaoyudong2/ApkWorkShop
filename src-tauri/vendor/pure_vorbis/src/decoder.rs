...
use crate::header::{Comments, FrameKind, Header};
use crate::mapping::Mapping;
use crate::mdct::Mdct;
use crate::mode::Mode;
use crate::residue::Residue;
use crate::util::Bits;
use crate::window::{OverlapTarget, Window, WindowRange, Windows};
...
impl Decoder {
    ...
    pub fn decode<R: BitRead>(&mut self, reader: &mut R) -> Result<Samples> {
        ...
        let packet_kind = r#try!(reader.read_u8_bits(1));
        ...
        let mode_idx = r#try!(reader.read_u8_bits(crate::util::Bits::ilog(mode_count as u8) as usize - 1)) as usize;
        ...
        if mode.frame_kind == FrameKind::Long {
            /* let is_prev_long_frame = */ r#try!(reader.read_bool());
            /* let is_next_long_frame = */ r#try!(reader.read_bool());
        }
        ...
        for (channel, floor_y_list) in self.floor_y_list.iter_mut().enumerate() {
            ...
            r#try!(floor.begin_decode(floor_y_list, reader, &self.setup.codebooks));
        }
...
impl DecoderBuilder {
    pub fn read_ident_packet<R: BitRead>(&mut self, reader: &mut R) -> Result<()> {
        self.header = Some(r#try!(PacketKind::Ident.read(reader, |r| Header::read(r))));
        Ok(())
    }

    pub fn read_comment_packet<R: BitRead>(&mut self, reader: &mut R) -> Result<()> {
        self.comments = Some(r#try!(PacketKind::Comment.read(reader, |r| Comments::read(r))));
        Ok(())
    }
    ...
    pub fn read_setup_packet<R: BitRead>(&mut self, reader: &mut R) -> Result<()> {
        ...
        self.setup = Some(r#try!(PacketKind::Setup.read(reader, |r| Setup::read(r, header))));
        Ok(())
    }
...
impl Setup {
    fn read<R: BitRead>(reader: &mut R, header: &Header) -> Result<Self> {
        let codebooks = r#try!(Self::read_codebooks(reader));

        r#try!(Self::skip_time_domain_trans(reader));

        let floors = r#try!(Self::read_floors(reader, codebooks.len()));

        let residues = r#try!(Self::read_residues(reader, codebooks.len()));

        let mappings = r#try!(Self::read_mappings(reader, header.channel_count(),
                                                floors.len(), residues.len()));

        let modes = r#try!(Self::read_modes(reader, mappings.len()));
    ...
    fn read_codebooks<R: BitRead>(reader: &mut R) -> Result<Vec<Codebook>> {
        let count = r#try!(reader.read_u8()) as usize + 1;
        let mut r = Vec::with_capacity(count);
        for _ in 0..count {
            let mut codebook = r#try!(Codebook::read(reader));
            codebook.idx = r.len();
            r.push(codebook);
        }
        Ok(r)
    ...
    fn skip_time_domain_trans<R: BitRead>(reader: &mut R) -> Result<()> {
        let len = r#try!(reader.read_u8_bits(6)) as usize + 1;
        for _ in 0..len {
            let value = r#try!(reader.read_u32_bits(16));
            if value != 0 {
    ...
    fn read_floors<R: BitRead>(reader: &mut R, codebook_count: usize) -> Result<Vec<Floor>> {
        let count = r#try!(reader.read_u8_bits(6)) as usize + 1;
        let mut floors = Vec::with_capacity(count);
        for _ in 0..count {
            let floor = r#try!(Floor::read(reader, codebook_count));
            floors.push(floor);
        }
        Ok(floors)
    }
    ...
    fn read_residues<R: BitRead>(reader: &mut R, codebook_count: usize) -> Result<Vec<Residue>> {
        ...
        for _ in 0..count {
            let residue = r#try!(Residue::read(reader, codebook_count));
    ...
    fn read_mappings<R: BitRead>(reader: &mut R, channel_count: usize,
        ...
        for _ in 0..count {
            let mapping = r#try!(Mapping::read(reader, channel_count, floor_count, residue_count));
    ...
    fn read_modes<R: BitRead>(reader: &mut R, mapping_count: usize) -> Result<Vec<Mode>> {
        ...
        for _ in 0..count {
            let mode = r#try!(Mode::read(reader, mapping_count));
            modes.push(mode);
        }
        if !r#try!(reader.read_bool()) {
...
impl PacketKind {
    fn read<BR: BitRead, R, F>(self, reader: &mut BR, f: F) -> Result<R>
        ...
        let packet_kind = r#try!(PacketKind::from_u8(r#try!(reader.read_u8()))
        ...
        r#try!(reader.read_exact(&mut magic));
        if magic != MAGIC {
