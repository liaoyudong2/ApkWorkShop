...
pub trait BitRead: Read {
    ...
    fn read_u32_bits(&mut self, len_bits: usize) -> Result<u32> {
        let (r, r_len) = r#try!(self.try_read_u32_bits(len_bits));
        if r_len == len_bits {
            Ok(r)
        } else {
    ...
    fn read_i32_bits(&mut self, len_bits: usize) -> Result<i32> {
        assert!(len_bits >= 2);
        let u = r#try!(self.read_u32_bits(len_bits - 1));
        let sign = r#try!(self.read_bool());
        if sign {
            Ok(-(u as i32))
        } else {
            Ok(u as i32)
...
impl<R: Read> BitReader<R> {
    ...
    fn fill_bit_buf(&mut self) -> Result<()> {
        ...
        let mut buf = [0; 4];
        let read = r#try!(self.inner.read(&mut buf));
