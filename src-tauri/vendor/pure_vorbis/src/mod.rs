...
macro_rules! enum_from_primitive {
    ($(#[$attr:meta])* $vis:vis enum $name:ident { $($variant:ident = $value:expr),* $(,)? }) => {
        $(#[$attr])*
        $vis enum $name {
            $($variant = $value),*
        }
...
pub use self::bitstream::{BitRead, BitReader};
pub use self::decoder::{ChannelIter, Decoder, DecoderBuilder, InterleavedSamplesIter, Samples};
