...
use crate::error::{Error, ErrorKind, ExpectEof, Result};
use crate::util::Bits;
...
impl Floor {
    pub fn read<R: BitRead>(reader: &mut R, codebooks_len: usize) -> Result<Self> {
        match FloorKind::from_u16(r#try!(reader.read_u16())) {
        ...
        let part_count = r#try!(reader.read_u32_bits(5)) as usize;
        if part_count == 0 {
        ...
        for _ in 0..part_count {
            let part_class = r#try!(reader.read_u8_bits(4));
        ...
        for _ in 0..class_count {
            let dim_count = r#try!(reader.read_u8_bits(3)) as usize + 1;

            let subclass_bit_count = r#try!(reader.read_u8_bits(2)) as usize;
            let master_book = if subclass_bit_count != 0 {
                let master_book = r#try!(reader.read_u8()) as usize;
            ...
            for _ in 0..subclass_books_count {
                let subclass_book = match r#try!(reader.read_u8()) as usize {
                    0 => None,
                    classbook_idx => {
        ...
        let mult = r#try!(reader.read_u8_bits(2)) + 1;
        let range = [256, 128, 86, 64][mult as usize - 1];
        let rangebits = r#try!(reader.read_u8_bits(4)) as usize;
        ...
        for &part_class in &part_classes {
            for _ in 0..classes[part_class].dim_count {
                let x = r#try!(reader.read_u16_bits(rangebits));
                if x_list.len() >= 65 {
    ...
    fn do_begin_decode<R: BitRead>(
        ...
        let non_zero = r#try!(reader.read_bool());
        if !non_zero {
            return Ok(());
        }

        let len_bits = crate::util::Bits::ilog(self.range - 1) as usize;
        result_y_list.push((r#try!(reader.read_u16_bits(len_bits)), true));
        result_y_list.push((r#try!(reader.read_u16_bits(len_bits)), true));
        for &part_class in self.part_classes.iter() {
            ...
            let mut cval = if cbits > 0 {
                ...
                r#try!(codebook.decode_scalar(reader)) as usize
            } else {
                0
            };
            for _ in 0..class.dim_count {
                ...
                cval >>= cbits;
                let y = r#try!(codebook.map(|c| c.decode_scalar(reader)).unwrap_or(Ok(0)));
