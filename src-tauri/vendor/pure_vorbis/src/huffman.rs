...
impl HuffmanDecoder {
    ...
    pub fn decode<R: BitRead>(&self, reader: &mut R) -> Result<u32> {
        ...
        let (mut code_bits, mut read) = r#try!(reader.try_read_u32_bits(lookup_len_bits));
        if read == 0 {
        ...
        let code = match entry {
            ...
            &LookupEntry::LongCode => {
                let r = r#try!(reader.try_read_u32_bits(self.max_code_len - lookup_len_bits));
                read += r.1;
                if read == 0 {
                    return Err(Error::Io(io::Error::new(io::ErrorKind::UnexpectedEof,
                ...
                r#try!(self.find_long_code(code_bits, read))
            },
...
impl HuffmanDecoderBuilder {
    pub fn create_code(&mut self, value: u32, len: usize) -> Result<()> {
        let code_straight = r#try!(self.next_code(len));
    ...
    fn next_code(&mut self, len: usize) -> Result<u32> {
        let r = r#try!(self.do_next_code(len));
    ...
    fn do_next_code(&mut self, len: usize) -> Result<u32> {
        ...
        let cur_code_bits = r#try!(self.do_next_code(idx)) << 1;
        self.cur_codes[idx] = Some(cur_code_bits);
        Ok(cur_code_bits)
    }
}
